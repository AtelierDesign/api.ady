  ...
  clients: [
    {
      ...
      redirect_uris: ['https://api.ady.world/api/auth/callback'],
      post_logout_redirect_uris: ['https://api.ady.world'],
...
module.exports = function createProvider(opts) {
  const issuer = 'https://api.ady.world/oidc/'';
  ...
  provider.use(async (ctx, next) => {
    ...
    if (ctx.oidc.route === 'end_session_success') {
      ctx.redirect('https://api.ady.world');
    }
  });
